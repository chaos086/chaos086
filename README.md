<!--apertura-->
<h1 align="center">❄️Jack Alejandro Marín Hernández❄️</h1>
<h2 align="center">Systems and Computer Engineer || Ingeniero en sistemas y computación</h2>
<h4 align="center">I am doesn't just write code... he builds systems with intention. || No solo escribo código… construyo sistemas con intención.</h4>

<p align="center">
  <img src="https://cdn.dribbble.com/userupload/42421979/file/original-2b972d38662cf2d12fede3d824f906bd.gif" width="300"/>
</p>

---
<!--Presentación-->

<h2 align="left"> 
  <img src="https://media.tenor.com/fSsxftCb8w0AAAAj/pikachu-running.gif" width="50"> Identity
</h2>

<img align="left"
src="https://c.tenor.com/OackI9-PD5sAAAAd/tenor.gif" 
width="300">

```java
public class Chaos086 {

    String name = "Jack Alejandro Marín Hernández";
    String alias = "Chaos";
    String role = "Systems and Computer Engineer";
    String location = "Colombia (CO)";
    String philosophy = "Code is logic... but it is also art.";
    String ethic = "Integrity is doing the right thing, even when no one is watching.";

    public String present() {
        return "Hello, my name is " + name +
               ", also known as \"" + alias + "\". " +
               "I am a " + role + " from " + location + ". " +
               philosophy + " " + ethic;
    }

    public static void main(String[] args) {
        Chaos086 user = new Chaos086();
        System.out.println(user.present());
    }
}
```
<!--Sobre mis-->
<h2 align="center">
  <img src="https://media.tenor.com/fSsxftCb8w0AAAAj/pikachu-running.gif" width="50"> About me
</h2>

<img align='right'
src="https://media1.tenor.com/m/MUAgDOboB1EAAAAC/vertical-discord-line.gif" 
width="25">

<img align='left'
src="https://media1.tenor.com/m/MUAgDOboB1EAAAAC/vertical-discord-line.gif" 
width="25">

```bash
I am passionate about software, music, literature and writing, video games, dance, physical activity, and sunsets.
I find in each of these spaces a form of expression and personal growth.

I consider myself a programmer with high potential, with a strong ability to learn, understand, and apply knowledge
effectively. I master various programming languages and possess solid programming logic, accompanied by good practices,
organization, and clarity in code development. I specialize in backend development, although I also have frontend skills.
My goal is to create efficient, scalable, and understandable applications that not only work well but are also
sustainable over time.

In the musical field, I have been an instrumentalist in the Youth Symphonic Band of Armenia, performing with the soprano
clarinet and bass clarinet. Music has been a fundamental part of my life. I am a fan of genres such as classical music,
boleros, rock, music in English and Spanish, dance music, openings, and opera, among many others.

Video games have also been a major influence on my personal and creative development. From a young age, I have had the
opportunity to enjoy titles such as Halo, GTA San Andreas, World of Warcraft, League of Legends, Warframe, Pokémon Ruby
and Sapphire, Fallout 3 and New Vegas, Minecraft, God of War, Dante’s Inferno, and Assassin’s Creed, among others. These
experiences have not only been entertainment but also a source of inspiration regarding the level of quality, detail, and
commitment required to create projects that can be considered true works of art, whether in software, web applications,
or any technological development.

Additionally, I am a passionate reader of the worlds that live within books. I deeply enjoy genres such as romance,
mystery, suspense, and horror, which fuel my imagination and strengthen my critical thinking. I always try to carry a
book with me, turning reading into a constant habit. This connection with literature has also inspired me to write,
allowing me to participate in and earn recognition in various competitions.

I define myself as an ethical, loyal, transparent, and committed professional, understanding that integrity and ethics
are fundamental pillars for building trust and carrying out anywork responsibly.
```
<!--Tecnologías-->

<h2 align="Center">
  <img src="https://media.tenor.com/fSsxftCb8w0AAAAj/pikachu-running.gif" width="50"> Technologys
</h2>


![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)
![JavaFx](https://img.shields.io/badge/javafx-%23FF0000.svg?style=for-the-badge&logo=javafx&logoColor=white)
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![HTML5](https://img.shields.io/badge/html5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)
![PHP](https://img.shields.io/badge/php-%23777BB4.svg?style=for-the-badge&logo=php&logoColor=white)
![CSS3](https://img.shields.io/badge/css3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)
![MySQL](https://img.shields.io/badge/mysql-4479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)
![MariaDB](https://img.shields.io/badge/mariadb-003545.svg?style=for-the-badge&logo=mariadb&logoColor=white)
![Apache](https://img.shields.io/badge/apache2-D22128?style=for-the-badge&logo=apache&logoColor=white)
![Git](https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJIDEA-000000.svg?style=for-the-badge&logo=intellij-idea&logoColor=white)
![VS Code](https://img.shields.io/badge/VSCode-0078d7.svg?style=for-the-badge&logo=visual-studio-code&logoColor=white)



<h4 align = "center">
  <p>
    <img src="https://projectpokemon.org/home/uploads/monthly_2021_01/large.d3d6544a14e4d67d-epic-pokemon-battle-tumblr.gif.71d51d13a87bca3ccea63ed257f27b82.gif">
  </p>
</h4>


<!--Redes Sociales-->

<h2 align="center">
  <img src="https://media.tenor.com/fSsxftCb8w0AAAAj/pikachu-running.gif" width="50"> Connect with me:
</h2>

<p align="center">
  <a href="https://www.facebook.com/jackmaster1185/?locale=es_LA">
    <img src="https://img.shields.io/badge/@JackMaster-1877F2?style=for-the-badge&logo=facebook&logoColor=white"/>
      </a>&nbsp;&nbsp    
  <a href="https://www.instagram.com/whitediamond086/">
    <img src="https://img.shields.io/badge/@WhiteDiamond086-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/>
      </a>&nbsp;&nbsp;
  <a href="https://steamcommunity.com/profiles/76561198819753001/">
    <img src="https://img.shields.io/badge/Steam-000000?style=for-the-badge&logo=steam&logoColor=white"/>
      </a>&nbsp;&nbsp;
  <a href="https://www.tiktok.com/@_jack.master_">
    <img src="https://img.shields.io/badge/@Jack.Master-000000?style=for-the-badge&logo=tiktok&logoColor=white"/>
      </a>&nbsp;&nbsp;
  <a href="https://open.spotify.com/user/31r4zc47wtydpziwovpf6ft36qb4?si=c16ad5bcc6a54817">
    <img src="https://img.shields.io/badge/Spotify-1DB954?style=for-the-badge&logo=spotify&logoColor=white"/>
      </a>&nbsp;&nbsp;
  <a href="https://github.com/chaos086">
    <img src="https://img.shields.io/badge/@chaos086-100000?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<h4 align = "center">
  <p>
    <img src="https://media1.giphy.com/media/v1.Y2lkPTc5MGI3NjExeHJheHFtZzhycGpqZXdsbGFzcXl3MHhka2sydTZvMGFzdDRheDlndSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/N4IfsN0Hio1Py/giphy.gif" 
      width="700">
  </p>
</h4>

<h4 align="center">𓆩✧𓆪 Requiescat in pace 𓆩✧𓆪</h4>

---
